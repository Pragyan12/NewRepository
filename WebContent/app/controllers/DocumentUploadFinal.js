		POTApp
				.controller(
					...
					function($scope, $http, $element, fileService, tempService,
							...
							quotationNoService, applicationLoggingService, documentPreviewService, UrlConstants) {
		...
		// preview section
		$scope.validationAndSubmit = function() {
		...
		function submit() {
			...
			var quoteNo = quotationNoService.get("quoteNo");
			...
			uploadDocFactory.uploadDoc().then(function(response) {
		...
		function imageUploadGetServ() {
			documentListFactory.getDocumentList().then(function(response) {
				...
				angular.forEach($scope.documentUploads,function(value, key) {
					$scope.documentUploads[key].mandatory = false;
					console.log($scope.documentUploads[key].mandatory);
				});
