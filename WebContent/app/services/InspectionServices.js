POTApp
		.factory(
				'InspectionServices',
				function($filter, StorageService, $q, $http,UrlConstants, cacheFactory) {
					return {
					...
					function fetchTimelist(currentDate) {
						...
						var inspectionDate = $filter('date')(currentDate,
								"dd/MM/yyyy");
						...
						angular.forEach(timeArray, function(value, key) {
							...
							var inspectionTime = new Date($filter('date')(
									new Date(inspectionDate), "dd/MM/yyyy"));
							inspectionTime.setHours(hours);
							inspectionTime.setMinutes(minutes);
							if (currentTime < inspectionTime) {
								inspectionTimeArray.push($filter('date')(
										new Date(inspectionTime), "hh:mm a"));
							}
						});
					...
					function defaultTime() {
						return [ '07:00 AM', '07:30 AM', '08:00 AM',
					...
					function getListOfTimes() {
						...
						var start = moment();
						start = moment(Math.ceil((+start) / ROUNDING)
								* ROUNDING);
					...
					function saveEnrichmentDetails() {
						...
						var data = {
								"quoteNo" : quoteNo, //formData.premiumInfo.premiumDetails.quoteNo,
							"CustomerDetails" : {
								...
								"stateName": formData.personalInfo.stateName,
								"stateCode": formData.personalInfo.stateCode,
							...
							"VehicleDetails" : {
								...
								"financierCity" : formData.nomineeFinancierInfo.financierCity,
								...
								"registrationNo" : formData.vehicleInfo.regNo1
										+ "-" + formData.vehicleInfo.regNo2
										+ "-" + formData.vehicleInfo.regNo3
										+ "-" + formData.vehicleInfo.regNo4
							},
						...
						var url = UrlConstants.MIS_CLOUD+UrlConstants.UPDATE_ENRICHMENT;
						var method="POST";
					...
					function getAllEnrichmentDetails(quoteNo) {
						...
						var method = "POST";
						cacheFactory.getJsonWithOutCache(method, url, data).then(function (response) {
							//console.log("Get Policy Information=> " + JSON.stringify(response.data.responseData));
							deferred.resolve(response);
						},function error(response){
							deferred.reject(response);
						});
